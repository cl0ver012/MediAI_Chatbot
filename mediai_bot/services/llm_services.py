...
class LLMService:
    ...
    def _do(self, model_type: ModelType, query_type, prompt: str, user: str, json_response: bool = False) -> str:
    ...
    def do(
        ...
        logger.debug(f"{user}: {query_type} {prompt}")
        return self.shadow_wrapper(model_type, shadow_models, self._do, query_type, prompt, user, json_response)
